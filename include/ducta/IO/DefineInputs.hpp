...
// 1) One entry: ("name", member) -> { "name", InputRef{ node.member } }
#define DUCTA_NODE_INPUT_PAIR_NODE(name, member) \
    { name, ::ducta::IO::InputRef{ node.member } }
...
template<>                                                                             \
struct NodeInputsTraits<NodeType>                                                      \
{                                                                                       \
    using Node = NodeType;                                                              \
    static std::map<std::string, ::ducta::IO::InputRef> const& get(Node& node)         \
    {                                                                                   \
        static std::map<std::string, ::ducta::IO::InputRef> inputs{                     \
            DUCTA_FOR_EACH(DUCTA_NODE_INPUT_PAIR_NODE, __VA_ARGS__)                     \
        };                                                                              \
        return inputs;                                                                  \
    }                                                                                   \
};
